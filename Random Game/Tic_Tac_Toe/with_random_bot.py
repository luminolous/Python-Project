...
def bot_move():
    availableMoves = [i for i in range(9) if i not in index_list]
    move = random.choice(availableMoves)
    index_list.append(move)
    return move
...
def main():
    ...
    player_2 = input('Input your name for Player 2 (If you want to play against the computer, type "bot"): ')
    ...
    for i in range(9):
        if i % 2 == 0:
            ...
            if player_2.lower() == "bot":
                index = bot_move()
                signDictionary[index] = 'O'
                print(f'Bot chose position {index + 1}')
            else:
